...
streamSaver.mitm = '/mitm.html?v=2';

export function AnonymousShareView({ sharedNodeId, setDownloadProgress }: { sharedNodeId: string, setDownloadProgress?: React.Dispatch<React.SetStateAction<any>> }) {
                ...
                try {
                  ...
                  if (node.rawRef?.provider === 'telegram' || node.telegramChannelId) {
                    ...
                    if (setDownloadProgress) {
                      setDownloadProgress({ status: 'downloading', progress: 0, fileName: node.name, isMinimized: false });
                    }
