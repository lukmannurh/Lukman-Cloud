...
const widget = `  const progressWidget = downloadProgress.status !== 'idle' ? (
...
c = c.replace(
    /[\s\S]*?\{\/\* Floating Download Progress Widget \*\/\}[\s\S]*?<\/div>\n      \)\}/m,
    function(match) {
        // We only want to remove the specific widget part
        const start = match.indexOf('{/* Floating Download Progress Widget */}');
        if (start !== -1) {
            return match.substring(0, start) + match.substring(match.lastIndexOf(')}') + 1);
        }
        return match;
    }
);

// Actually, simpler regex for removing the old widget
c = c.replace(/      \{\/\* Floating Download Progress Widget \*\/\}[\s\S]*?<\/div>\n      \)\}\n/m, '');
